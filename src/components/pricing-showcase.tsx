  ...
  return (
    ...
    >
      ...
      <div className="mt-6 flex items-end gap-3">
        <span className="text-5xl font-semibold tracking-tight text-[var(--color-ink)]">{couplePlan.price}</span>
        <span className="pb-1 text-sm leading-5 text-black/55">{couplePlan.priceLabel}</span>
      </div>
      <p className="mt-2 text-sm text-[var(--color-moss)]">Perfect for one private wedding or event.</p>

      <MarketingButtonLink href="/signup?intent=couple" tone="ink" className="mt-6 w-full rounded-[18px] py-4">
        {couplePlan.ctaLabel}
      </MarketingButtonLink>

      <div className="mt-7 border-t border-black/8 pt-6">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-black/45">Includes</p>
        <div className="mt-4 space-y-3">
          {couplePlan.features.map((feature) => (
