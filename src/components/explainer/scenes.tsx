...
// Scene timing and layout are unchanged so output matches the validated source.

import React from "react";
import { Easing, clamp, useTime, useSprite, SpriteContext } from "./animations";
import {
  CFI, PaperBackground, StripedPlate, PhotoCard, PhoneFrame, QRPlate,
...
// ── LocalSprite — start/end relative to the parent sprite's localTime ───────
...
// ═══════════════════════════════════════════════════════════════════════════
const PHONE_LAYOUT = [
  { x: 200, y: 120, w: 124, rot: -8, hue: "warm", delay: 0.05, caption: "zdravica — Lina" },
  { x: 875, y: 5, w: 100, rot: 6, hue: "paper", delay: 0.25, caption: null },
  { x: 1560, y: 130, w: 128, rot: 10, hue: "cool", delay: 0.10, caption: "prvi ples — Adi" },
  { x: 80, y: 460, w: 138, rot: 4, hue: "paper", delay: 0.35, caption: null },
  { x: 1700, y: 450, w: 130, rot: -7, hue: "warm", delay: 0.20, caption: "torta — Tina" },
  { x: 170, y: 790, w: 134, rot: 9, hue: "cool", delay: 0.45, caption: null },
  { x: 860, y: 870, w: 132, rot: -5, hue: "paper", delay: 0.55, caption: "Nana, mutna ali ❤" },
  { x: 1580, y: 810, w: 128, rot: 7, hue: "warm", delay: 0.40, caption: null },
  { x: 490, y: 240, w: 110, rot: -12, hue: "cool", delay: 0.60, caption: null },
  { x: 1340, y: 270, w: 112, rot: 11, hue: "paper", delay: 0.65, caption: "djeca na podu" },
];

function MiniPhone({ w = 130, hue = "paper", label = "IMG", src }) {
  return (
    <div style={{
      width: w,
      height: w * (852 / 420),
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      <div style={{
        ...
        <Drift x={4} y={3} speed={0.4} rotate={0.4}>
          <PhotoCard
            ...
            label="Professional gallery · 247 photos"
            hue="warm"
            caption={
              <>
                <div style={{
      ...
      {PHONE_LAYOUT.map((p, i) => {
        ...
        const targetX = 960 - p.w / 2;
        const targetY = 540 - p.w * (852 / 420) / 2;
      ...
      <LocalSprite start={0.6} end={3.6}>
        <div style={{
          ...
          <Display size={56} style={{ textAlign: "center" }}>
            Fotografov album je <em style={{ fontStyle: "italic", color: CFI.accent }}>savršen</em>.
          </Display>
        </div>
      </LocalSprite>
      ...
      <LocalSprite start={6.0} end={10.6}>
        <div style={{
          ...
          <div style={{
            ...
            boxShadow: "0 16px 40px rgba(23,32,51,0.10)",
          }}>
            <Display size={64} style={{ textAlign: "center", lineHeight: 1.08 }}>
              Ali noć je živjela na<br />
              <em style={{ fontStyle: "italic", color: CFI.accent }}>još 47 telefona.</em>
            </Display>
          </div>
        </div>
      </LocalSprite>
      ...
      <LocalSprite start={14.8} end={18.8}>
        <div style={{
          ...
          <Display size={56}>
            Jedan link. <em style={{ fontStyle: "italic", color: CFI.accent, fontFamily: "\"Playfair Display\"", fontSize: "83px" }}>Jedna galerija.</em>
          </Display>
        </div>
      </LocalSprite>
    </div>
...
export function UploadScene() {
      ...
      <div style={{
        ...
        <div style={{ height: 4, borderRadius: 2, background: "rgba(255,250,242,0.18)", overflow: "hidden" }}>
          <div style={{
            ...
            transition: done ? "background 0.3s" : "none",
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
        ...
        {({ localTime, duration }) => {
          ...
          return (
            <div style={{
              ...
              opacity: op, transform: `translateY(${(1 - Math.min(1, localTime / 0.4)) * 10}px)`,
            }}>
              <Kicker>Korak 1 od 1</Kicker>
              <div style={{ height: 8 }} />
              <Display size={58} style={{ lineHeight: 1 }}>
      ...
      <LocalSprite start={1.6} end={9.5}>
        {({ localTime, duration }) => {
          const op = localTime < 0.5 ? Easing.easeOutCubic(localTime / 0.5)
            : duration - localTime < 0.5 ? Easing.easeInCubic((duration - localTime) / 0.5) : 1;
          return (
            <div style={{ position: "absolute", left: 0, right: 0, bottom: 36, textAlign: "center", opacity: op }}>
              <Caption size={22} style={{ maxWidth: 900, margin: "0 auto" }}>
                Skeniraj QR. Ubaci sve sa večeri. <em style={{ fontStyle: "italic", color: CFI.ink }}>Bez aplikacije, bez registracije.</em>
              </Caption>
            </div>
          );
        }}
      ...
      <ConfettiBurst x={960} y={540} count={80} start={6.2} end={9.3} scale={1.3} />
    </div>
  );
}

// ═══════════════════════════════════════════════════════════════════════════
// SCENE 5 — GALLERY WALL
// ═══════════════════════════════════════════════════════════════════════════
const GALLERY_TILES = [
  { section: "Vjenčanje", label: "maja", hue: "warm" },
  { section: "Vjenčanje", label: "sara", hue: "paper" },
  { section: "Vjenčanje", label: "tom", hue: "cool" },
  { section: "Vjenčanje", label: "lina", hue: "warm" },
  { section: "Veselje", label: "adi", hue: "paper" },
  { section: "Veselje", label: "leila", hue: "cool" },
  { section: "Veselje", label: "iva", hue: "warm" },
  { section: "Veselje", label: "kim", hue: "paper" },
  { section: "Torta", label: "tina", hue: "cool" },
  { section: "Torta", label: "pat", hue: "warm" },
  { section: "Zdravice", label: "sam", hue: "paper" },
  { section: "Zdravice", label: "nana", hue: "cool" },
];
...
export function GalleryScene({ titleOverride, dateOverride, kickerOverride } = {}) {
  ...
  const sections = [
    { name: titleOverride ? null : "Vjenčanje · 64", yRow: 0 },
    { name: titleOverride ? null : "Veselje · 138", yRow: 1 },
    { name: titleOverride ? null : "Torta i zdravice · 41", yRow: 2 },
  ];
    ...
    <div style={{ position: "absolute", inset: 0 }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 18 }}>
          <Display size={64}>
            {titleOverride || <>Lejla <span style={{ color: CFI.accent, opacity: "0.5", lineHeight: "1.8" }}>&amp;</span> Amar</>}
          </Display>
          <Caption size={18} style={{ fontFamily: CFI.mono, letterSpacing: "0.1em", textTransform: "uppercase" }}>
      ...
      {GALLERY_TILES.map((tile, i) => {
        ...
        const y = startY + row * (tileH + gap + (row > 0 ? 30 : 0));
        const appearAt = 0.7 + i * 0.18;
        ...
        return (
          <div key={i} style={{
            ...
            opacity: appT, borderRadius: 8, overflow: "hidden",
            boxShadow: "0 12px 28px rgba(23,32,51,0.10), 0 1px 3px rgba(23,32,51,0.06)",
          }}>
            <StripedPlate label={`${tile.section.slice(0, 4)} · ${tile.label}`} hue={tile.hue} src={ASSETS.gallery[i]} />
          </div>
        );
      })}
      ...
      {sections.map((s, i) => {
        ...
        const y = startY + i * (tileH + gap + (i > 0 ? 30 : 0)) - 28;
        const appearAt = 1.2 + i * 0.8;
        const op = clamp((t - appearAt) / 0.5, 0, 1);
        return (
          <div key={i} style={{ position: "absolute", left: startX, top: y, opacity: op }}>
            <span style={{
              fontFamily: CFI.mono, fontSize: 13, letterSpacing: "0.18em",
              textTransform: "uppercase", color: CFI.moss,
            }}>{s.name}</span>
          </div>
        );
      })}
      ...
      <LocalSprite start={6.0} end={9.5}>
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: 999, padding: "22px 44px", border: `1px solid ${CFI.rule}`,
            boxShadow: "0 20px 50px rgba(23,32,51,0.18), 0 2px 8px rgba(23,32,51,0.06)",
          }}>
            <Display size={56}>
              Svaki trenutak. <em style={{ fontStyle: "italic", color: CFI.accent }}>Jedna privatna galerija.</em>
            </Display>
          </div>
        </div>
      </LocalSprite>
...
export function EventTypesScene() {
  ...
  const subs = [
    ...
    { start: 4.8, end: 7.6, kicker: "Sve što vrijedi pamtiti", title: <>Tvoj <span style={{ color: CFI.accent }}>dan</span>, na jednom mjestu</>, date: "23 . 05 . 26", srcs: ASSETS.generic },
  ];
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      <div style={{
        ...
        <Display size={44} style={{ color: CFI.ink }}>
          Vjenčanja · Rođendani · Party — <em style={{ fontStyle: "italic", color: CFI.accent }}>sve što vrijedi pamtiti.</em>
        </Display>
      </div>
    </div>
  );
...
export function MiniGalleryPreview({ kicker, title, date, localT, srcs = [] }) {
  const tiles = [
    { hue: "warm" }, { hue: "paper" }, { hue: "cool" },
    { hue: "paper" }, { hue: "warm" }, { hue: "cool" },
  ];

  const panY = -Easing.easeInOutSine(localT) * 12;

  return (
    <div style={{ position: "absolute", left: "50%", top: 110, transform: `translateX(-50%) translateY(${panY}px)` }}>
      <div style={{
        background: "rgba(255,250,242,0.92)", backdropFilter: "blur(6px)",
        borderRadius: 24, padding: 40,
        boxShadow: "0 28px 60px rgba(23,32,51,0.16), 0 4px 12px rgba(23,32,51,0.08)",
        border: `1px solid ${CFI.rule}`, width: 1180,
      }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, marginBottom: 26 }}>
          <Kicker>{kicker}</Kicker>
          <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
            <Display size={52}>{title}</Display>
            <span style={{
              fontFamily: CFI.mono, fontSize: 14, letterSpacing: "0.14em",
              textTransform: "uppercase", color: CFI.inkSoft,
            }}>{date}</span>
          </div>
        </div>
...
export function OutroScene() {
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      <ConfettiBurst x={960} y={540} count={120} start={0.0} end={3.0} scale={1.8} />
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: CFI.display, fontSize: 180, fontWeight: 500,
          letterSpacing: "-0.025em", color: CFI.ink, fontStyle: "italic",
        }}>
          C<span style={{ color: CFI.accent }}>o</span>nfetti
        </div>
        <div style={{ fontFamily: CFI.sans, fontSize: 28, color: CFI.inkSoft, letterSpacing: "0.02em" }}>
          svaki trenutak, <em style={{ fontStyle: "italic", color: CFI.accent }}>jedna galerija.</em>
        </div>
        <LocalSprite start={1.6} end={5}>
          <SceneWrap fadeIn={0.5} fadeOut={0.4}>
            <div style={{
              marginTop: 16, fontFamily: "Georgia",
              color: CFI.ink, textTransform: "uppercase",
              border: `1px solid ${CFI.rule}`, borderRadius: 999,
              background: "rgba(255,250,242,0.6)",
              width: "556px", height: "18px", letterSpacing: "3.3px",
              fontSize: "21px", lineHeight: "1.3", padding: "13px 22px 15px",
            }}>
              confetti.app
            </div>
          </SceneWrap>
